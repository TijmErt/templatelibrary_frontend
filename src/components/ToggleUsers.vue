...
import { GET_USERS } from '../graphql/userGraphQL'

// Define a reactive variable to store the fetched users
const Users = ref<{ id: string, userName: string }[]>([])
const selectedUserId = ref<string | null>(null)
const isUsersListVisible = ref(true) // Track visibility of user list
...
const selectUser = (userId: string) => {
  ...
  // Optionally, update localStorage with the selected user ID (or the entire user data)
  ...
  <div class="toggleUserContainer" v-if="isUsersListVisible">
    ...
    <ul v-if="!queryLoading && !queryError">
      <li
        ...
        :style="{ cursor: 'pointer', padding: '2px', backgroundColor: selectedUserId === user.id ? '#45a049' : '' }"
      >
        {{ user.userName }}
      </li>
    </ul>
